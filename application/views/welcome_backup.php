...
public function refreshmap()
	{
		//* set session to hold the particular button that got clicked, so it dosent get clikde again
		    $worker = $this->session->userdata('clicked');
	        $this->db->where('job', $worker);
	    	$query = $this->db->get('workforce'); 
	...
	public function call_bottom()
	{
	    ...
	    $this->db->where('job', $worker_job);
	    	$query = $this->db->get('workforce'); 
         $rate = $this->Main_model->get_ratings($worker_job);
			...
			foreach($query->result() as $row)
		    {
		    	...
		    	echo 
		    	'
<div id="job_detail'.$row->id.'" class="w3-white " style="height: 60vh;min-height:60vh; z-index: 1000; width:100%; display: none; position: fixed; border-top-left-radius:15px;border-top-right-radius:15px; bottom: -1px;min-height:350px; margin:auto auto auto auto; ">
 <div id=""  class="w3-bottom " style="position: relative;  height: 50vh; width:100%; ">
    <div class=" w3-border-bottom w3-border-light-grey purple'.$row->id.' " style="height: 52px; width: inherit;display: flex;">
        <div class="first'.$row->id.'" style="font-weight: ; font-size: 1.3rem; margin:10px 10px;">Hire '.$row->firstname.'</div>
        <div class="response'.$row->id.'" style="font-weight: 100; font-size: 1.3rem; margin:10px 10px;"></div>
        <div id="times'.$row->id.'" class="w3-hover-text-grey w3-text-black" style="position: absolute; right:0; display: block; font-weight: 50; font-size: 1.3rem; margin:10px 10px;cursor:pointer;">&times</div>
        <div id="back'.$row->id.'" onclick="back()" class="w3-hover-text-grey w3-text-black w3-small" style="position: absolute; display: none; right:0;padding:18px 10px; font-weight: 50; font-size:  margin:10px 10px;">Back</div>
    </div>
<!--COVER BEGINS--> <div id="w_cover'.$row->id.'" class="" style="display: block;">
    <div class="w3-white w3-container">
      <p class="w3-text-black w3-center w3-small">Before Proceeding, please view the facebook profile of the worker</p>
      <div class="shadow1 "  style="margin-top:10px;height: 60px;display: flex; width:100%; padding: 1px 5px;">
        <div class="" style="flex:2;">
          <p class="w3-medium" style="font-weight: 600;">Worker Rating</p>
          <div class=" w3-small" style="color: #c8d415;margin-top:-10px;"><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><strong class="w3-text-grey" style="margin:0 0 0 10px;">'.$rating.'</strong> </div>
        </div>
          <div class="" style="flex:2;">
          <p class="w3-medium" style="font-weight: 600;">+ / - comments</p>
      ...
      <div class=" shadow1 w3-border-light-grey"  style="height: 60px; width:100%;display:flex; padding: 1px 5px;margin-top:5px;">
            <div class="" style="flex:2;">
        <p class="w3-medium" style="font-weight: 600;">Workmanship Range </p>
        <div class=" w3-small w3-text-green" style="margin-top: -10px;">&#8358<span class=" w3-text-grey " style="padding: 5px 9px; margin: -15px 0 0 -5px;">  '.$row->w_from.' - '.$row->w_to.' </span></div>
          ...
          <div class="" style="flex:2;">
        <p class="w3-mwdium" style="font-weight: 600;">Job Type </p>
        <div class=" w3-small w3-text-green" style="margin-top: -10px;"><span class=" w3-text-grey " style="padding: 5px 9px; margin: -15px 0 0 -5px;">'.$row->job.' </span></div>
          </div>
          
      </div>

      <div class=" w3-border-bottom w3-border-light-grey"  style="height: 52==60px; width:100%; padding: 1px 5px;margin-top:5px; display: flex;">
        <div class=" "  style="flex: 2;">
          <p class="w3-medium" style="font-weight: 600;">Facebook Profile</p>
          <div class=" w3-small w3-text-blue" style="margin-top: -10px;"><i class="fa fa-facebook-square w3-round-large"></i> <strong class="w3-text-grey" style="margin-left: 5px;"> Profile</strong>  <span id="view_facebook'.$row->id.'" class="w3-text-purple  w3-round-xxlarge w3-hover-text-light-grey" style="margin-left:10%;padding:3px 5px;">View</span></div>
        </div>

        <div class=" "  style="flex:2;">
          <p class="w3-medium" style="font-weight: 600;">Jobs Completed</p>
     ...
     <div id="proceed'.$row->id.'"  class="w3-purple w3-btn w3-bottom w3-round-xlarge w3-medium" style=" bottom:5px; max-width: 280px;left:50%; margin:auto 0 auto -140px;">Proceed</div>
<!--COVER ENDS--> </div>


<!--h_COVER BEGINS--> <div id="h_cover'.$row->id.'" style="display: none;" class="w3-container contact">
  <!--|||||||||||||||||||||||||||||||||||||||||||||||||||||||START FULLPAGE LOADER||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||-->
  <div id="loader" style="position: fixed;display: none;" class="w3-modal">
    <div class=" w3-transparent w3-center">
      <span style="position: absolute;top:45%;left:45%;" class=" fa-3x w3-text-white fa fa-spinner fa-spin"></span>
    </div> 
  </div>
  

   <div class="w3-display-container"  style="height: 300px;width:100%;min-height:300px; padding: 1px 5px;overflow: scroll; ">
    <div class="" style="position: relative; height: 25vh;">
       <div class="w3-text-black w3-small" style="font-weight: 600;">JOB DETAILS</div>
      <div class="w3-border w3-border-purple w3-round-large" style="display: flex;margin:2px 0 0 0;">
          ...
          <input id="name'.$row->id.'" name="name" class="w3-input w3-border-0  w3-medium " placeholder="Your First and Last name" style="outline:none;flex: 16" type="text">
      </div>
      <div class="w3-border w3-border-purple w3-round-large " style="display: flex;margin:2px 0 0 0;">
      <div class=" w3-margin w3-transparent" style="flex: 1;"><i class="fa fa-pencil w3-text-purple w3-transparent " aria-hidden="true"></i></div>
      <input id="email'.$row->id.'" name="email" class="w3-input w3-border-0  w3-medium" placeholder="Email address" style="outline:none;text-decoration:none;flex: 16;" type="email">
      </div>
      <div class="w3-border w3-border-purple w3-round-large "style="display: flex;margin:2px 0 0 0;">
      <div class=" w3-margin w3-transparent" style="flex: 1;"><i class="fa fa-phone w3-text-purple w3-transparent " aria-hidden="true"></i></div>
      <input id="phone'.$row->id.'" name="phone" class="w3-input w3-border-0 w3-medium " placeholder="Active Phone Number" style="outline:none;flex: 16;" type="tel">
      </div>
    ...
    <div style="position:relative;top:50px;height: 16vh;">
     <span id="contents'.$row->id.'" contenteditable="true" class="w3-white w3-round-large w3-border-bottom w3-border" style="display: inline-block;outline: none; margin:0 0 5px 0 ;min-height:50px;font-size:14px; width: 100%;max-height:250px; padding: 2px 5px 0 5px">    <span id="inside'.$row->id.'" class="w3-medium w3-margin-bottom w3-text-grey" >Describe your job in detail</span></span>
     </div>
    </div>
    
    <div id="hire'.$row->id.'"  class="w3-purple w3-btn w3-large w3-round-xlarge" style="position:fixed;bottom:5px; width: 280px;left:50%; margin:auto 0 auto -140px;">Hire</div>
 
<!--COVER ENDS--> </div>
<div id="r_res'.$row->id.'" style="display:none;" class="r_cover">
 ...
 echo
   	"
   		 <script>
   		  $('#contents".$row->id."').click(function(){
   		      $('#inside".$row->id."').hide();
   		      $('#contents".$row->id."').text('');
   		  })
   		  $('#view_facebook".$row->id."').click(function(){
   		    var fb = '//".$row->facebook_page."'
   		      window.open(fb); 
   		  });
		  $('#proceed".$row->id."').click(function(){
   		...
   		";
   	   		...
   	   		echo 
   		"<script>
        $('#hire".$row->id."').click(function(){
        var name = $('#name".$row->id."').val();
        var contact = $('#phone".$row->id."').val();
        var email = $('#email".$row->id."').val();
        ...
        if(name == ''){
            ...
            alert('describe your job');
        }else if(email == ''){
            alert('insert your email');
        }else{
    $.ajax({
            url: 'Welcome/hiredescription', 
            method: 'POST',
            data: {con:con, name:name, contact:contact, worker:worker, job:job, skill:skill, email:email},
            beforeSend: function(){ 
              $('#loader').show();
            },
	...
	public function hiredescription()
	{
	    ...
	    $end_time = date('Y-m-d H:i:s', strtotime('+10 minute', strtotime($dat)));
	  	if ($this->session->userdata('user')) {
	  	    $data = $this->Main_model->insert_description($name, $contact, $con, $ip, $bro, $worker, $job, $end_time, $skill, $dat);
			echo 'true';
		}else{
	         ...
	         $this->db->where('user_id', $this->session->userdata('user'));
    	      ...
    	      if($query->num_rows() < 1){
    			$datas = $this->Main_model->insert_user($name, $contact, $ip, $bro);	
    		   }
    		 $data = $this->Main_model->insert_description($name, $contact, $con, $ip, $bro, $worker, $job, $end_time, $skill, $dat);
			echo 'true';
		}
	    ...
	    foreach ($query->result_array() as $value){
	    	echo'
			 ...
			 <div class="  w3-container w3-margin-top" style="position: relative;border:0.5px solid light-grey; height: 45px; width:100%;">
				    ...
				    <div class="w3-small w3-text-grey" style="position: absolute; bottom: 0px;">STATUS: Incomplete</div>
			</div>
			<hr>
			 ';
			echo "
			...
			$('#go".$value['id']."').click(function(){
				...
				alert('success');
				$('.response-layout').load('".base_url()."Welcome/response', {wor:wor, jor:jor});
...
<div id="pay'.$row->id.'" class="w2-modal " style="display: none!important;padding:3px;background-color: rgb(0 0 0 / 33%)!important;z-index:1100;">
 <div class="w3-display-topright close_edit w3-text-red w3-hover-text-black w3-hide" style="font-size: 30px;font-weight:700;padding:10px;">&times</div>
  <div class="w3-center w3-modal-content   w3-animate-zoom  w3-round-large w3-white " style="margin-top:20%;padding:2px;background-color:#1a121d;">
    <div class=" w3-display-container w3-round-large " style="position: relative;height:450px; width:100%;">
      
      <div class="" style="text-align: left;padding:2px 5px;">
        <h4 class="w3-center" style="font-weight: 600;">How do you want to pay ?</h4>


                  <div id="" class="w3-container w3-center" style="position:relative; display: block; height: 100%;z-index:1100;">
                  <h5 id="how_to_pay" class="d-t-black2" style="position: relative;left:; text-align: left; font-weight: 300;font-size:16px; margin:5px 0 0 0;">Its safer to make payments to DADI, DADI will go lengths to ensure that your job is completed. </h5>
                  <div id="payworker'.$row->id.'" class="w3-mobile  w3-center w3-medium  w3-border-purple w3-hover-purple w3-border  w3-round-large" style="height:45px;display:block; width:150px;position: relative; top:20px; padding:12px 0 0 0;">Pay to worker </div>
                  <form class="w3-show"> 
                     <div id="paydadi'.$row->id.'" onClick="payWithRave()" class="  w3-center w3-medium w3-mobile w3-orange w3-border w3-border-orange w3-hover-orange w3-text-white w3-btn w3-hover-text-white w3-round-large" style="height:45px; display:block; width:150px;position: relative; top:30px; padding:12px 0 0 0;">Pay to DADI</div>
                  </form>
                 
                </div>

                <div class="w3-small d-t-black2 w3-center" style="position: absolute;width:90%;margin:auto 5%; bottom: 30px; font-weight:bold;">Please dont close the modal or leave the page</div>
 ...
 <div id="closeme'.$row->id.'" class="" style="position: fixed; bottom:0; height: 60vh; width:100%;  border-top-left-radius:15px;border-top-right-radius:15px;">
    <div class="w2-card w3-border-bottom w3-border-light-grey  " style="height: 52px; width: inherit;display: flex; border-top-left-radius:15px;border-top-right-radius:15px;">
    <div id="ur'.$row->id.'" class="w3-medium w3-text-black" style="font-weight: bold;margin:10px 5px ; display:block;padding:4px 5px ;">Your Price: &#8358 <strong  class="worker_price'.$row->id.'" style="">'.$row->amount.' </strong></div>
      <div id="rate'.$row->id.'" class="w3-medium w3-text-black w3-center" style="font-weight: bold;margin:10px 5px ; display:none;">Rate this Worker</div>
      <div id="call_nd_chat'.$row->id.'" class="w3-medium w2-t-black w3-center" style="font-weight: bold;margin:10px 5px; padding:4px 5px ;display:none;">Meet Up</div>        
      </div>
...
<div class="w3-light-grey w3-white" style="position: relative;height: 55vh;">

<!--***********************************************HANDLE TYPICALLY REPLIES AND COUNTDOWN************************************************-->
...
<!--************************************************************HANDLE CALL AND WHATSAPP****************************************************************-->
  <div id="call'.$row->id.'" class="w3-container w3-center w3-white w3-animate-left" style="position:relative; display: none; height: 100%;">
  <div class="w3-small d-t-black2 w3-margin-top" > If a worker dosent deliver a nice job, <b>click</b> not satisfied and DADI will get someone else to do this job for you</div>
    <div class=" w3-card-6 w3-center w3-small w3-hover-purple d-t-black w3-round-large" style="height:40px; width:150px;position: absolute; top:25%; left:50%; margin:0 0 0 -75px;padding:12px 0 0 0;">Call Worker <i class="fa fa-phone" style="position:relative; left:10px; font-size:17px;"></i></div>
     <div class=" w3-card-6 w3-center w3-small w3-btn w3-round-large w3-green" style="height:40px; width:150px;position: absolute; top:39%; left:50%; margin:0 0 0 -75px;padding:12px 0 0 0;">Chat On Whatsapp<i class="fa fa-whatsapp" style="position:relative; left:10px; font-size:17px;"></i></div>
      <div id="notcomplete'.$row->id.'"  class="w3-btn w3-bottom w3-round-xlarge w3-border d-t-black w3-border-purple" style="position:relative;display:block; top:200px; max-width: 250px; margin:auto; ">Not Satisfied</div>
      <div id="complete'.$row->id.'"  class="w3-btn w3-bottom w3-round-xlarge w3-purple" style="position:relative;color:white; display:none; top:210px; max-width: 250px;  ">Job Completed</div>
  </div>
  ...
  <div id="star'.$row->id.'" class="w3-container w3-center w3-white w3-animate-left" style="position:relative; display: none; height: 100%;">
    <div class="  w3-center w3-small  w3-text-black w3-round-large" style="height:40px; width:280px;position: absolute; top:3%; left:50%; margin:0 0 0 -140px;padding:12px 0 0 0;">
      <div class=" w3-white w3-round-large" style="position:relative; height: 30vh; width: 90%; margin: 20px auto;">
        ...
        <p class="w3-center w3-medium" style="position: relative; padding:0 15px 0 25px; margin: auto auto auto -50px; top:px; left:50%; display: flex;height: 20px; width: 100px;"><span class="star_count'.$row->id.'" style="margin:0 5px 0 0;">0</span> Star</p>

      <input class="w3-input w3-medium comment'.$row->id.' w3-margin-bottom"  style="position: relative; top:48px; margin:auto 5%; width:90%; outline: none; " type="text" name="comment" placeholder="comment on this job">
...
<!--************************************************************************WHEN WORKER DOSENT REPLY, SHOW THISMESSAGE****************************************-->
  <div id="no_reply'.$row->id.'" class="w3-container w3-center w3-white w3-animate-left" style="border-left:2px solid pink; display: none; height: 20vh;">
    <p id="" style="text-align:left;">'.$name->firstname.' hasn`t replied you yet, feel free to cancel this hire, or give him a little more time</p>
...
<!--********************************************SAY HE IS REVIEWING YOUR JOB AND GIVE HIM THE QUOTED AMOUNT****************************************-->
  <div id="review'.$row->id.'" class="w3-container w3-center w3-white w3-animate-left" style="position:relative; display: none; height: 100%;">
    ...
    <p id="" style="text-align:center;">Typically replies within 10 minutes</p>
    <h2 id="dem'.$row->id.'" style="text-align:center;font-weight: ;">5000:3982</h2>
  </div>
<!--*****************************************************START FULLPAGE LOADER************************************************************************-->
  <div id="loader" style="position: fixed;display: none;top:50%; height:50vh; min-height:50vh;" class="w3-modal">
    <div class=" w3-transparent w3-center">
      <span style="position: absolute;top:45%;left:45%;" class=" fa-3x w3-text-white fa fa-spinner fa-spin"></span>
    </div> 
  </div>
 <!--*******************************************************END FULLPAGE LOADER**************************************************************************--> 
  ...
  <div id="quote'.$row->id.'" class="w3-container  w3-white w3-animate-left" style="border-left:2px solid blue;margin:0 0 0 0;  display: none; height: 100px;">
...
<!-- ******************************************************************CANCEL AND COMPLETE-->
  ...
  <div id="cancel_hires'.$row->id.'"  class="w3-btn missed w3-bottom w3-round-xlarge w3-red w3-large" style="position:relative;color:white; bottom:-40px; max-width: 280px;left:50%; margin:auto 0 auto -140px;display:none;"> Cancel Hire</div>

<!-- ********************************************************************2 BUTTONS-->
  <div id="two_but'.$row->id.'" class="w3-container  w3-margin-top w3-animate-left" style="position:relative; top:10px; display: none; height: 100%;">
...
//HANDLE ACCEPT AND DECLINE BEFORE ANYOTHER THING
            	if(data.accept == 1 && data.complete > 0)
                {
                  $('#call".$row->id."').hide();
                  $('#star".$row->id."').show();
                  $('#wait".$row->id."').hide();
            	...
            	else if(data.accept == 1 && data.complete == 0)
                {
				  ...
				  $('#cancel_hires".$row->id."').hide();
                	...
                	alert('you have declined this job, feel free to choose someone else');
                	window.location.href = '".base_url()."map'; 
                }else if(data.accept == 0 && data.complete == 0 && data.cancel == 1)
                {
                    $('#no_reply".$row->id."').show();
                    $('#cancel_hires".$row->id."').show();
                }else{
               ...
               $('#no_reply".$row->id."').hide();
                  ...
                  $('#wait".$row->id."').hide();           
                  $('#quote".$row->id."').show();
                  $('#two_buts".$row->id."').show();
                  $('#negotiate".$row->id."').show();
                  $('#cancel_hires".$row->id."').hide();
                } 
                 if(data.worker_t > 0 && data.user_t > 0 && data.nego == 1 && data.decline > 0 )
                {
                $('#typically".$row->id."').hide();
                  $('#no_reply".$row->id."').hide();
               	 $('#review".$row->id."').hide(); 
...
;                  $('#star".$row->id."').hide();
       ...
       $('#notcomplete".$row->id."').click(function(){
              var track = '".$row->job_id."';
              $.ajax({
                url:  'Welcome/notcomplete',
                method: 'POST',
                data: {track:track},
                beforeSend: function(){
    ...
    $('#payworker".$row->id."').click(function(){
              var track= '".$row->job_id."';
              var pays= 'worker';
            	$.ajax({
		            url:  'Welcome/pay',
		            method: 'POST',
		            data: {track:track, pays:pays},
		            beforeSend: function(){
		              $('#payworker".$row->id."').hide();
		              $('#loader').show();
		              alert('paying');
		            },
		            success: function(data)
    ...
    $('#cancel_hires".$row->id."').click(function(){
         var track= '".$row->job_id."';
              alert('amount');
             $.ajax({
                url:  '".base_url()."Welcome/cancel',
                method: 'POST',
                data: {track:track},
                beforeSend: function(){
                  $('#loader').show();
...
<script>
//****************************LETS CHECK EVERY SECOND FOR IF PAYMENT HAS BEEN set **********************
setInterval(function(){
    $.ajax({
        url:  'Welcome/check_ifpayment',
        method: 'POST',
            success: function(data)
            {
              if(data === 'show'){
...
<script >
  ...
  function payWithRave() {
        var amount = $('#amount".$row->id."').val();
        var x = getpaidSetup({
            PBFPubKey: 'FLWPUBK_TEST-cefcefa51846593a0f8ffcb6ab8c12db-X',
...
";
  ...
  public function accept()
    {
        ...
        $this->session->set_userdata('pay_set', 'isset');
        $cut = $amount * 0.10;
    ...
    $this->session->unset_tempdata('workker');
    $this->session->unset_tempdata('jobber');
    $this->session->set_tempdata('cancel', 'true', 30);
    ...
    {
    $job = $this->input->post('track');
    $pay = $this->input->post('pays');
    $data = array(
            'pay_who' => $pay
    );
    $this->db->where('job_id', $job);
    $this->db->update('hire', $data);
    $this->session->unset_userdata('pay_set');
    echo 'success';
    }
    ...
    {
        $this->session->set_tempdata('not_complete', 'true', 70);
    }


    public function check_ifpayment()
    {
      if($this->session->userdata('pay_set') == 'isset'){
        echo "show";
      }  else{
        echo "hide";
      }     
  ...
  public function finish()
  {
    $this->session->unset_tempdata('workker');
        $this->session->unset_tempdata('jobber');
        $this->session->unset_userdata('isset');
